CLASS zcl_fill_zs4d401_2950_flights DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_fill_zs4d401_2950_flights IMPLEMENTATION.
...
* Read highest connection number for each flight
    ...
    FIELDS carrier_Id, connection_id, flight_date, price, currency_code, plane_type_id
     WHERE connection_id = ( SELECT MAX( connection_id )
                               FROM /dmo/flight
                              WHERE carrier_id = main~carrier_id )
       AND flight_Date = ( SELECT MIN( flight_date )
                             FROM /dmo/flight
                            WHERE carrier_id = main~carrier_id AND
                                  connection_id = main~connection_id )
...
*Add 50 new connection numbers and 2000 days of flights for each
    LOOP AT max INTO DATA(line).
      DO 50 TIMES.
        ...
        DATA(repetitions) = COND i( WHEN line-carrier_id = 'LH' AND
                                         line-connection_id = '0405'
                                         THEN 4000
                                         ELSE 2000 ).
    ...
    DELETE ADJACENT DUPLICATES FROM insert_tab
        COMPARING carrier_id connection_id flight_date.


    INSERT zs2950_fligths FROM TABLE @insert_tab.

    out->write( |Generated { sy-dbcnt } rows in table zs2950_fligths| ).
