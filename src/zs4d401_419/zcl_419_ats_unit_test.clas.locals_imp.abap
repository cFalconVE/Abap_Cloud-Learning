...
CLASS lcl_passenger_flight DEFINITION .
  ...
  PRIVATE SECTION.
    ...
    CLASS-DATA lt_flights_buffer TYPE TABLE OF st_flights_buffer.
...
*      INTO TABLE @DATA(keys).
    ...
    FIELDS carrier_id, connection_id, flight_date,
           ...
           price, currency_code
     WHERE carrier_id    = @i_carrier_id
      INTO TABLE @DATA(lt_flights_buffer).

*    LOOP AT lt_flights_buffer INTO DATA(key).
    LOOP AT lt_flights_buffer INTO DATA(flight).
...
*    IF lt_flights_buffer IS INITIAL.
*      SELECT
*        FROM /lrn/passflight
*      FIELDS carrier_id, connection_id, flight_date,
*             plane_type_id, seats_max, seats_occupied, price, currency_code
*       WHERE carrier_id = @i_carrier_id
*        INTO @DATA(flights_buffer).
*    ENDIF.

    TRY.
        DATA(ls_flight_raw) = lt_flights_buffer[ carrier_id    = i_carrier_id
...
*      INTO @DATA(flight_raw).
          INTO CORRESPONDING FIELDS OF @ls_flight_raw.
    ENDTRY.

*      IF sy-subrc = 0.
    IF ls_flight_raw IS NOT INITIAL.
      me->carrier_id    = i_carrier_id.
      me->connection_id = i_connection_id.
      me->flight_date   = i_flight_date.

      planetype  = ls_flight_raw-plane_type_id.
      seats_max  = ls_flight_raw-seats_max.
      seats_occ  = ls_flight_raw-seats_occupied.
      seats_free = ls_flight_raw-seats_max - ls_flight_raw-seats_occupied.
...
* convert currencies
      TRY.
          cl_exchange_rates=>convert_to_local_currency(
            EXPORTING
              date              = me->flight_date
              foreign_amount    = ls_flight_raw-price
              foreign_currency  = ls_flight_raw-currency_code
              local_currency    = me->currency
            IMPORTING
              local_amount      = me->price
          ).
        CATCH cx_exchange_rates.
          price = ls_flight_raw-price.
      ENDTRY.
