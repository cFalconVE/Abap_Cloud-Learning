CLASS zcl_2950_oos_intf_usage DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_2950_oos_intf_usage IMPLEMENTATION.
...
*        DATA(carrier) = NEW lcl_carrier( i_carrier_id = c_carrier_id ).
        ...
        DATA(carrier_2) = lcl_carrier=>get_instance(  i_carrier_id = c_carrier_id ).

        out->write( name = `Carrier Overview`
                    data = carrier->get_output( ) ).

        out->write( name = `Carrier 2 Overview`
                    data = carrier_2->get_output( ) ).
