...
*           TO r_result.
    ...
    lv_text = 'Flight &carreid& &conneid& on &date& from &from& to &to&'(005).

    lv_text = replace( val = lv_text sub = '&carreid&' with = carrier_id ).
    lv_text = replace( val = lv_text sub = '&conneid&' with = connection_id ).
    lv_text = replace( val = lv_text sub = '&date&'    with = |{ flight_date DATE = USER }| ).
    lv_text = replace( val = lv_text sub = '&from&'    with = connection_details-airport_from_id ).
    lv_text = replace( val = lv_text sub = '&to&'      with = connection_details-airport_to_id ).

    APPEND lv_text TO r_result.
