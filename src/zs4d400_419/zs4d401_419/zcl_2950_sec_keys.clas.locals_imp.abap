CLASS lcl_flights DEFINITION.
  ...
  PRIVATE SECTION.
    DATA connections TYPE SORTED TABLE OF zs2950_fligths
            WITH NON-UNIQUE KEY carrier_id connection_Id flight_date.

    DATA connections_sk TYPE SORTED TABLE OF zs2950_fligths
            WITH NON-UNIQUE KEY carrier_id connection_id flight_date
...
CLASS lcl_flights IMPLEMENTATION.
  ...
  METHOD constructor.
    SELECT FROM zs2950_fligths FIELDS * INTO TABLE @connections.
    SELECT FROM zs2950_fligths FIELDS * INTO TABLE @connections_sk.
  ENDMETHOD.
  ...
  METHOD read_secondary_1.
    LOOP AT connections_sk INTO DATA(connection)
        USING KEY k_plane
        WHERE plane_type_id = '737-800'.
    ENDLOOP.
  ENDMETHOD.
